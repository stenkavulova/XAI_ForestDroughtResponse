...
# The main thing I want to try out here is generating Shapley values for a bigger dataset.
# I train the model on a subset, but I can generate Shap values for a larger dataset.
# Idea is to map Shap values. 
...
#plt.savefig('D:/Stenka_Cliwac/Topic_1/12_PYTHON/20230705_XGB/20230705_confusion_matrix2.png', dpi=300, bbox_inches='tight')
...
# Not just the 4000 points in the dataset used for a train/test split. 

df_all = pd.read_csv("D:/Stenka_Cliwac/Topic_1/04_PROCESSED_DATA/20230705_modeling_df_unbalanced/modeling_df_2class_2019_unbal.csv")

print(df_all["NDVI_categories"].value_counts())
# NDVI_categories
# no_change         5217
#large_decrease    2491
...
# Prepare the data by separating the predictors (X) and the target variable (y)
X_all = df_all.drop(["x", "y", "NDVI_categories", "Year_NDVI_anom", "NDVI_anomaly"], axis=1) # predictors 
...
shap_values = explainer.shap_values(X_all) #X are all predictors (before the train/ test split)
...
# 23 predictors (each predictor has a column) # (7708, 23)
...
# Create a new DataFrame with "x" and "y" columns from df_all
df_new = df_all[["x", "y"]].copy()

# Add the "most_important_predictor" column
df_new["most_important_predictor"] = most_important_predictor_per_row

#df_new.to_csv("D:/Stenka_Cliwac/Topic_1/05_RESULTS/20230705_shapley_map_dfs/20230705_most_important_predictor.csv", index=False)

#%% Pure shap values 

# Create a DataFrame from shap_values with column names from X_all
shap_df = pd.DataFrame(shap_values, columns=X_all.columns)

# Add "x" and "y" columns from df_all at the beginning
shap_df.insert(0, "x", df_all["x"])
shap_df.insert(1, "y", df_all["y"])


#shap_df.to_csv("D:/Stenka_Cliwac/Topic_1/05_RESULTS/20230705_shapley_map_dfs/20230705_shap_values.csv", index=False)
...
shap.dependence_plot("agriculture_proximity",  shap_values = shap_values, features = X_all, interaction_index = None, ax = ax, show = False)
