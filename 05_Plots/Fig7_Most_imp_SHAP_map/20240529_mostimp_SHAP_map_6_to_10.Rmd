...
# Import mean SHAP dataframes across all years of #6 to #10 most important predictors 

```{r import mean SHAP dfs }

tic("load mean SHAP df of the 5 most important predictors")

shap_df_path_forest_prox = "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240423_mean_SHAP_dfs/forest_prox_mean_SHAP_df.csv"
shap_df_forest_prox <- data.table::fread(shap_df_path_forest_prox, select = c(1,2,8), header = TRUE)

shap_df_path_ET_sameyear = "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240423_mean_SHAP_dfs/ET_sameyear_mean_SHAP_df.csv"
shap_df_ET_sameyear <- data.table::fread(shap_df_path_ET_sameyear, select = c(1,2,8), header = TRUE)

shap_df_path_LST_sameyear = "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240423_mean_SHAP_dfs/LST_sameyear_mean_SHAP_df.csv"
shap_df_LST_sameyear <- data.table::fread(shap_df_path_LST_sameyear, select = c(1,2,8), header = TRUE)

shap_df_path_SSM_sameyear = "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240423_mean_SHAP_dfs/SSM_sameyear_mean_SHAP_df.csv"
shap_df_SSM_sameyear <- data.table::fread(shap_df_path_SSM_sameyear, select = c(1,2,8), header = TRUE)

shap_df_path_airtemp_1year = "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240423_mean_SHAP_dfs/airtemp_1year_mean_SHAP_df.csv"
shap_df_airtemp_1year <- data.table::fread(shap_df_path_airtemp_1year, select = c(1,2,8), header = TRUE)

toc()

tic.clearlog()

glimpse(shap_df_SSM_sameyear)

# load mean SHAP df of the 6-10 most important predictors: 172.4 sec elapsed
# glimpse(shap_df_SSM_sameyear)
# Rows: 10,439,063
# Columns: 3
...
colnames(shap_df_forest_prox)[3] <- "forest_prox"
colnames(shap_df_ET_sameyear)[3] <- "ET_sameyear"
colnames(shap_df_LST_sameyear)[3] <- "LST_sameyear"
colnames(shap_df_SSM_sameyear)[3] <- "SSM_sameyear"
colnames(shap_df_airtemp_1year)[3] <- "airtemp_1year"

glimpse(shap_df_airtemp_1year)
# x 
# y
# airtemp_1year
# It worked!
...
combined_df <- cbind(shap_df_forest_prox[, c("forest_prox")], shap_df_ET_sameyear[, c("ET_sameyear")], 
                     shap_df_LST_sameyear[, c("LST_sameyear")], shap_df_SSM_sameyear[, c("SSM_sameyear")], 
                     shap_df_airtemp_1year[, c("airtemp_1year")])
...
coords_df <- shap_df_airtemp_1year[, 1:2]
...
# [1] "airtemp_1year" "ET_sameyear"   "forest_prox"   "LST_sameyear"  "SSM_sameyear"       

levels_imp = data.frame(ID=1:5, most_important_predictor = c("airtemp_1year", "ET_sameyear", "forest_prox", "LST_sameyear", "SSM_sameyear"))
...
# Save raster
writeRaster(mostimp_lonlat, "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240430_mostimp_SHAP_Fig6/mostimp_predictor_6_to_10_raster.tif", overwrite = TRUE)

# Load raster and check
raster <- rast("D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240430_mostimp_SHAP_Fig6/mostimp_predictor_6_to_10_raster.tif")
plot(raster) 
# looks fine!
...
write.table(imp_df_coords, file = "D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240430_mostimp_SHAP_Fig6/mostimp_predictor_df_6_to_10.csv", row.names = FALSE, col.names = TRUE)

# Load mostimp predictor table and check
test <- data.table::fread("D:/Nextcloud/Documents/CliWaC/05_RESULTS/20240430_mostimp_SHAP_Fig6/mostimp_predictor_df_6_to_10.csv", header = TRUE)
glimpse(test)
# looks fine!
